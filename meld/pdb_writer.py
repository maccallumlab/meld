...
footer = 'TER\nEND\n\n'
template = ('ATOM  {atom_number:>5d} {atom_name:4s} {residue_name:>3s} ' +
            '{residue_number:5d}    {x:8.3f}{y:8.3f}{z:8.3f}')


class PDBWriter(object):
    def __init__(self, atom_numbers, atom_names, residue_numbers,
                 residue_names):
        ...
        self._atom_names = [''.join([' ', atom_name])
                            if len(atom_name) < 4 else atom_name
    ...
    def get_pdb_string(self, coordinates, stage):
        ...
        zipper = zip(self._atom_numbers, self._atom_names,
                     self._residue_numbers, self._residue_names,
                     range(coordinates.shape[0]))
        lines = [self.template.format(atom_number=atom_num,
                                      atom_name=atom_name,
