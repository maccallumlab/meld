...
def create_elastic_network_restraints(
    ...
    k: force constant
    write_file: Optional parameter to output restraints to a text file. Useful for comparing restraints generated by different functions
    """
    ...
    if write_file:
        with open(write_file, 'w') as f:
            for pair, dist in zip(close_pairs, close_distances):
                i, j = pair[0], pair[1]
                if abs(i - j) >= residue_separation:
                    f.write(f"{residues[i].index} {restrained_atom} {residues[i].name} {residues[j].index} {restrained_atom} {residues[j].name} {dist} {k_val} \n")
