...
class DensityManager:
    ...
    def add_density(
        self,
        filename,
        blur_scaler: scalers.BlurScaler,
        threshold=None,
        scale_factor=None,
    ):
        try:
        ...
        density = DensityMap(
            density_data, origin, voxel_size, blur_scaler, scale_factor, threshold
        )
...
class DensityMap:
    def __init__(
        self, density_data, origin, voxel_size, blur_scaler, scale_factor, threshold
    ):
        ...
        if blur_scaler._scaler_key_ == "constant_blur":
            ...
            tmp_pot = np.matrix.flatten(
                self.map_potential(tmp_pot, threshold, scale_factor)
            )
            self.density_data = np.array(
                [tmp_pot.tolist()] * blur_scaler._num_replicas
            ).astype(np.float64)
        elif blur_scaler._scaler_key_ == "linear_blur":
            density_data = np.matrix.flatten(density_data)
            for i in np.linspace(
                blur_scaler._min_blur, blur_scaler._max_blur, blur_scaler._num_replicas
            ):
