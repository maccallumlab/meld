...
class TestInitState(unittest.TestCase):
    def setUp(self):
        ...
        self.coords = numpy.zeros((self.N_ATOMS, 3))
        self.vels = numpy.zeros((self.N_ATOMS, 3))
        self.spring_states = numpy.zeros(self.N_SPRINGS)
        self.lam = 0.5
        self.energy = 42.
        self.spring_energies = numpy.zeros(self.N_SPRINGS)

    def test_should_raise_with_coords_not_2d(self):
        "should raise RuntimeError if coords is not 2d"
        bad_pos = numpy.zeros((75, 83, 52))
    ...
    def test_spring_energies_must_match_spring_states(self):
        ...
        bad_spring_energies = numpy.zeros(42)
        with self.assertRaises(RuntimeError):
