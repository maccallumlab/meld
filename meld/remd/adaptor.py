...
class FluxAdaptor(AcceptanceCounter):
    ...
    def _make_f_monotonic(self, f: np.ndarray) -> np.ndarray:
    ...
    def _apply_smoothing(self, f: np.ndarray) -> np.ndarray:
        smooth = np.linspace(0, 1, self.n_replicas)
        return (1.0 - self.smooth_factor) * f + self.smooth_factor * smooth

    def _compute_g(
        self, f: np.ndarray, previous_lambdas: List[float]
    ) -> Tuple[np.ndarray, np.ndarray]:
        gy = np.array(previous_lambdas)
        gx = f
        return gx, gy

    def _compute_new_lambdas(self, gx: np.ndarray, gy: np.ndarray) -> np.ndarray:
        f = interpolate.interp1d(gx, gy)
        samples = np.linspace(0, 1, self.n_replicas)
        return f(samples)


Adaptor = Union[NullAdaptor, EqualAcceptanceAdaptor, FluxAdaptor]
