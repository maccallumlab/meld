    ...
    def __init__(
        ...
        if self.active:
            for r in self.restraints:
                self.alignment_map[r.alignment].append(r)
    ...
    def add_interactions(
        ...
        if self.active:
            ...
            for alignment in self.alignment_map:
                force = _create_rdc_force(alignment)
...
def _create_rdc_force(alignment_index):
    force = mm.CustomCompoundBondForce(
        ...
        comp1 = rdc_{alignment_index}_s1 * (rx^2 - ry^2);
        comp2 = rdc_{alignment_index}_s2 * (3 * rz^2 - r^2);
        comp3 = rdc_{alignment_index}_s3 * 2 * rx * ry;
        comp4 = rdc_{alignment_index}_s4 * 2 * rx * rz;
        comp5 = rdc_{alignment_index}_s5 * 2 * ry * rz;
        r = distance(p1, p2);
        """,
    )
    for i in range(5):
